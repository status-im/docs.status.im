...
Changes to the documentation can be made on GitHub by editing files in the [`source`](./source) folder. Please make sure that any changes you make are against `develop`. The idea is that we can merge recklessly to `develop`, then anyone see if the changes look OK on `dev.status.im`. 
